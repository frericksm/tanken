(defproject tanken-service "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "tanken-service.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'tanken-service.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init tanken-service.service/service #'tanken-service.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} tanken-service.server)
